...
public class Tank extends Vehicle implements ISelectable {
    ...
    public void setPlasmaCount(int plasmaCount) {
        this.plasmaCount = plasmaCount;
    }
...
//            setY(getPrevy());
        ...
        if (tmp instanceof Plasma) {
            if (((Plasma) tmp).getMissileOwner() != this && lastMissileToHit != tmp) {
                setShoulddie();
                lastMissileToHit = (Plasma) tmp;

            }
        }else if (tmp instanceof Missile) {
