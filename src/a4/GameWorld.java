...
public class GameWorld implements IObservable, IGameWold {
    ...
    public void firePlayerTankPlasma() { // Fire player tank
        boolean ableToFire = myTank.firePlasma(); // First check to see if the tank is able to fire
        if (ableToFire) { // if so, create a new missile object and decrease the missile count of players tank
            Plasma m = new Plasma(myTank);
            myTank.setPlasmaCount(myTank.getPlasmaCount() - 1);
            go.add(m); // add the newly created missile to the collection of game objects.
            if (sound) fireMissile.play();
        } else {
            System.out.println("This tank has no more ammo"); // Print error message.
...
//                System.out.println("Checking for collisions");
    ...
    public int[] getAmoCount(){
        int[] myReturn = new int[3];
        myReturn[0] = myTank.getMissileCount();
        myReturn[1] = myTank.getPlasmaCount();
        myReturn[2] = myTank.getGrenadeCount();
        return myReturn;
